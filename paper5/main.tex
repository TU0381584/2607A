...
genuinely wide distribution rather than one being wrong. Together these
...
cluster sizes $N \in \{7, 19\}$ under ring and hex-grid adjacency. We
label these four campaigns M2 (centralised), M3 (federated/privacy), M4
(disruption), and M6 (cluster-size scaling) throughout, matching this
project's internal milestone numbering (M5 is this paper's own
evaluation-integrity reframe, not a separate results campaign); a
fifth, earlier milestone (M1, live-testbed recalibration) motivates the
offline stress environment of Section~\ref{sec:system-model} but
reports no results of its own here.
...
not run a full campaign with it here: at the time of the M2/M3
campaigns we believed our topology treated every gNB as an
...
the M2/M3 campaigns had deliberately controlled -- but that correction
came after M2/M3 were run, so no FedProx sweep is reported in this
paper; it is left as the natural next step now that genuine,
deliberately-controlled client heterogeneity is available as an
...
paper uses the offline stress environment (3, 7, or 19 gNBs depending
on the campaign); none of it is a live claim, and
Section~\ref{sec:system-model} explains why we do not believe an
offline-trained ranking here would transfer live without separate
verification. Federated learning across three gNBs
