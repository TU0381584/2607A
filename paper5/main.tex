...
\caption{30-seed campaign, three arms. (a) The established SLA-compliance
metric, where GAT-CTDE's advantage is present but statistically noisy.
(b) Mean reward per step, the correctness-aware metric
(Section~\ref{sec:correctness-metrics}): GAT-CTDE's advantage is
tighter and more decisive on the metric that reflects the actual
training objective.}
...
\caption{Federated GAT-CTDE. (a) Federation cost on mean reward per
step: centralized training retains a small, borderline-significant
edge over federated training with no privacy noise. (b) Privacy-utility
curve using block precision, the metric that survives the
compliance-based curve's non-monotonic, uninterpretable pattern at the
same noise levels: precision is perfect through $\sigma=1.0$ and drops
sharply by $\sigma=4.0$.}
