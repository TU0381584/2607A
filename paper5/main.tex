...
before it could be reported as a finding.
...
of which gNB or slice it belongs to (centralized training), while
action selection at rollout time consults only the acting agent's own
embedding row (decentralized execution). The per-request TD
...
\begin{algorithm}[t]
\caption{GAT-CTDE: centralized training, decentralized execution}
\label{alg:gat-ctde}
\begin{algorithmic}[1]
...
\FOR{episode $= 1, 2, \dots$}
  ...
  \FOR{step $t = 1, \dots, T$}
    \STATE $E_t \gets f_\theta(X_t, A)$ \COMMENT{one embedding row per gNB; centralized: sees every node}
    \FOR{each pending request $r$ at gNB $i$, slice $k$}
      \STATE $a_r \gets \arg\max_a Q_{\phi_k}(E_t[i])[a]$ w.p.\ $1{-}\epsilon$, else uniform random \COMMENT{decentralized: own row $E_t[i]$ only}
    \ENDFOR
...
are made by a freshly-initialized, never-trained copy of its own policy
...
from the joint state -- there is no FL-specific desynchronization left
...
each episode's own seeded RNG, not fixed to one gNB.
...
normalize by each step's own already-logged pending-request count
before averaging (mean reward per pending request), built from data
every run already logs, and use this normalized metric as the primary
...
\caption{M4 disruption campaign, 10 seeds/arm. (a)-(b) Normalized
...
the environment synthesizes for the following step. All three are
...
Fig.~\ref{fig:m4-disruption}(a)-(b) report the normalized-reward cost at
...
multiplier for every arm. The normalized-reward signal (not shown) that
...
volume normalization above, which only corrects the dominant,
...
its precise location is not a property we would claim generalizes
...
append bug and an unseeded weight-initialization path,
...
unseeded weight-initialization path could not have caught on their own
...
tradeoff. Evaluating the same frozen policies under mid-episode gNB
