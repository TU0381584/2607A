...
cluster-size-specific effect distinct from anything at $N=3$, though
GAT-CTDE's own exact collapse rate does not yet replicate cleanly
between seed samples (31\% vs.\ 78\%), reported as an open question
rather than resolved. Together these
...
\caption{$N=19$ cluster, per-seed evidence. (a) Collapse rate (fraction
of eval runs with zero blocks), pooled across all three topologies,
primary sample (seeds 900--911) against the independent replication
sample (1000--1002) -- single-agent DQN collapses totally in both;
GAT-CTDE's own rate is markedly higher in the replication sample.
(b) Block precision by topology, primary sample, non-collapsed seeds
only (single-agent DQN has none to show): GAT-CTDE's non-collapsed
seeds are mostly near-ceiling but with a low-precision seed at every
topology, while independent DQN sits in a narrow, mediocre band,
identical across topologies as expected for an arm that never consumes
adjacency.}
...
close to its own established $\sim$30\% (9/30) collapse rate at $N=3$
(Section~\ref{sec:results-m2}), suggesting cluster size does not by
itself drive GAT-CTDE's collapse rate up. The same probe on GAT-CTDE's
...
\subsection{Independent-Seed Replication: One Finding Holds, One Does Not}
...
primary sample's 0.29--0.49 range, still topology-invariant). GAT-
...
cleanly differentiates either -- but GAT-CTDE's exact collapse rate at
$N=19$ is not yet a number we are prepared to quote with confidence: at
$n=3$ per topology this could be sampling variance (if the true rate
is near 30\%, three-for-three collapsed has roughly a 3\% chance by
itself, unlikely but not disqualifying), or it could mean the primary
sample's reading does not generalise as cleanly as it first appeared.
We report the range rather than either number alone, for the same
reason Section~\ref{sec:results-m4}'s independent-DQN churn retraction
does: an unresolved discrepancy honestly reported is more useful to a
reader than a single sample's point estimate presented as settled.
...
primary sample and 78\% on the replication one, a discrepancy we report
as open rather than resolve by picking whichever number arrived first.
...
distinct from anything at $N=3$; but replication on an independent seed
sample left GAT-CTDE's own exact collapse rate an open question rather
than a settled number, reported as such rather than smoothed over.
...
most direct remaining step. Three narrower questions this paper's own
...
undisrupted baseline); why the privacy threshold's exact location
...
or a factor this paper has not identified; and pinning down GAT-CTDE's
exact collapse rate at $N=19$ with more seeds specifically aimed at
that question, ahead of the broader topology-sparsity and reward-margin
axes Section~\ref{sec:results-m6} already found to be weak or null. A
further, not yet run, extension is a FedProx sweep under genuine
per-gNB load heterogeneity -- newly available as a controlled variable
because Section~\ref{sec:results-m6}'s own investigation found every
M2--M4 result already carries \emph{implicit}, uncontrolled per-gNB
load heterogeneity (a default in the frozen KPM-source constructor
neither this paper nor its predecessors had deliberately set), which a
homogeneous-load override now makes possible to isolate as its own
experimental variable.
