...
magnitude -- traced to a concrete unit mismatch between the offline
demand model and the live scheduler's byte-denominated backlog counter,
not left as an unexplained gap. Together these
...
The offline/live gap here is not evidence that live traffic is
millions of times more severe than anything offline training exposed a
policy to; it is a unit-calibration mismatch between two sources that
share a field name but not a physical scale, never reconciled because
$L_{\max}$ was tuned only against the offline proxy. This is a
different failure mode from Section~\ref{sec:collapse}'s compliance-
metric finding or Section~\ref{sec:results-m6}'s OR-aggregation
finding -- not a metric that structurally rewards degenerate behaviour,
but one whose numeric scale was never checked for consistency across
the two sources it is computed identically from. We report it plainly
rather than silently patching $L_{\max}$ post hoc: doing so would
require deciding what live backlog scale is representative without
much more live data than this bounded anchor collected, and the
decision-level finding this section's live evidence actually supports
-- that the trained policy's targeting survives contact with real
traffic -- does not depend on it.
