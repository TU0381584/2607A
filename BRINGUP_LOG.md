...
**Not yet done (at the time of the entry above):** the actual M8 goal (evaluating the offline-trained M2/M7 policies against this live testbed) is a distinct next step from "the rig is live again," and had not been started.

### Continuation, same session: real traffic, contention gate re-verified live

**The machine rebooted mid-session** (user-confirmed, intentional, likely to finish applying the swap resize) -- the whole stack above (core network, gNB, both UE namespaces) was wiped and rebuilt from scratch, identically to the procedure just described, verified clean at every step again (connectivity + `dmesg` OOM/segfault check after each component). Not re-narrated in full here since it is a repeat of the same steps; the only difference worth recording is memory ran tighter throughout this second pass (as low as 165-373Mi free at various points, vs. 181-508Mi the first time) since the post-reboot desktop session came back up alongside it, and stayed stable regardless (zero OOM/segfault signatures across both passes).

- **E2 telemetry re-verified via `qoe_oran_framework/scripts/probe_e2_preconditions`**: P2 (wire protocol) PASS; P4 shows `dl_mac_buffer_occupation` at 0% population with idle-attached UEs (matches the addendum's own finding -- this field needs real load); P3 shows all 3 UEs flat at the 5.00 PRB idle baseline.
- **Real per-slice traffic generated** via the pre-existing `experiments/traffic/run_traffic_profiles.sh` (embb: sustained UDP 4Mbps/1200B on UE1; urllc: sustained UDP 300Kbps/100B on UE3; mmtc: bursty UDP 50Kbps/80B, 2s-on/6s-off, on UE2 -- targets the already-running `iperf3-target` container, auto-detects each UE's current PDU-session IP rather than hardcoding it). All three streams confirmed genuinely flowing (sustained 0% packet loss over 50s+ in both `embb.log`/`urllc.log`, mmtc's bursts landing cleanly each cycle) -- an initial `pgrep` check suggesting embb/urllc had died was a broken grep pattern on my part, not a real problem, caught by reading the actual log contents rather than trusting the one-line check.
- **Re-ran P3/P4 with traffic flowing**: `dl_mac_buffer_occupation` now shows real, non-zero backlog (26.7% urllc, 33.3% embb, 0% mmtc -- consistent with mmtc's tiny intermittent bursts not accumulating a queue) and `avg_prbs_dl` demand genuinely elevated for the two sustained streams (embb: mean 14.93/max 19 PRB vs. idle's flat 5; urllc: mean 5.13/max 9) while mmtc stays at the idle baseline between bursts, as expected.
- **Contention gate re-run against embb** (`experiments/scripts/phase1_contention_gate.py --sd 16777215`, 15/30/15-poll baseline/pinned/recovery windows): pinning embb's ceiling to `min=max=1` moved mean backlog from 329 (baseline) to 6,602,435 (pinned, max 10,023,782) -- **PASS** on the script's own defined criterion (`pinned_mean > baseline_mean and pinned_max > 0`). Trace: `experiments/logs/phase1/embb_contention_gate_20260819_230328.jsonl`.
  - **Honest caveat, not gated by the script's verdict but worth recording**: the 15s recovery window (after restoring `min=1/max=4`) did not show backlog draining -- it continued climbing to a mean of 10,023,785, essentially flush against the pinned phase's own max. Plausible, mundane explanations not yet distinguished: the window may simply be too short for a backlog this large to drain, and/or the restored ceiling (`max=4`) may itself still be tight relative to embb's real ~4Mbps demand. Not treated as a red flag (the gate's actual pass criterion doesn't depend on it), but not silently omitted either.

**State at end of this entry**: core network + gNB + 3 UEs live, real per-slice traffic flowing, contention gate passed. The gNB's embb ceiling is currently sitting at the contention gate's own last command (`min=1/max=4`), a fixed test value -- not something any trained policy has set. **Still not done**: an actual live-vs-offline evaluation needs a new control loop that loads a trained M2/M7 checkpoint and drives real `slicing_control_m` actions from the policy's own decisions; no such loop exists yet, and building one is flagged as a distinctly larger, riskier task (new code, closed-loop control of a live core network) than anything completed so far.
