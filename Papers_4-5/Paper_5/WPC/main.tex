...
This section's own live anchor uses the SLA-only reward and, at 20--24 episodes per campaign, is powered to detect a collapse but not to carry a formal significance claim. A companion submission~\cite{cacs26-paper} independently confirms the same offline-to-live transfer for a QoE-aware reward variant on the same rig, at 128 episodes per arm with correction for multiple comparisons, and separately measures the E2 control loop itself as negligible overhead (median round-trip 0.57\,ms) from a policy under 20,000 parameters. The two campaigns use different reward formulations and different sample sizes, but arrive at the same qualitative conclusion, that an offline-trained decision policy retains its behaviour once deployed against real traffic, from two independent live campaigns on this rig rather than one.
