...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, inner sep=3pt, minimum height=0.8cm, minimum width=2.2cm},
  simbox/.style={box, fill=blue!8},
  livebox/.style={box, fill=orange!10},
  evalbox/.style={box, fill=green!10},
  warnbox/.style={box, fill=red!8, dashed, minimum width=2.9cm},
  arrow/.style={-{Latex[length=2.2mm]}, thick},
  dashedarrow/.style={-{Latex[length=2.2mm]}, thick, dashed},
]
\node[simbox] (sim) {Closed-loop offline\\simulator (Alg.~1)};
\node[simbox, right=0.6cm of sim] (train) {DQN training\\6 seeds $\times$ 300 ep};
\node[simbox, right=0.6cm of train] (ckpt) {Frozen\\checkpoint};
\node[livebox, right=0.6cm of ckpt] (rig) {Live OAI gNB\\(E2 control loop)};
\node[evalbox, right=0.6cm of rig] (liveeval) {Live evaluation\\11 seeds, $\leq$46 ep/arm};
\node[evalbox, right=0.6cm of liveeval] (stats) {Statistical analysis\\Fisher exact, SLA\%, MOS};

\draw[arrow] (sim) -- (train);
\draw[arrow] (train) -- (ckpt);
\draw[arrow] (ckpt) -- (rig) node[midway, above, font=\tiny] {deploy, frozen weights};
...
\end{tikzpicture}
\caption{End-to-end methodology. Checkpoints trained offline (left) are deployed live with no further learning (centre); a held-out offline evaluation runs alongside the live pipeline (bottom) but, per Section~III-C, does not predict its outcome.}
\label{fig:pipeline}
\end{figure*}
