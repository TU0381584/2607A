...
re-evaluation (21 episodes per arm) shows DQN under the QoE-aware
reward sustains 100\% SLA compliance throughout, while the SLA-only
reward and both static arms converge to a statistically
indistinguishable, lower rate (19 of 21 episodes fully compliant for
each, Fisher exact $p=1.0$): the collapse-avoidance edge our
earlier, smaller-sample evaluation suggested for the SLA-only reward
does not clearly hold at this larger scale, though it does for the
QoE-aware one. We additionally live-evaluate checkpoints trained for
...
Each of the four arms was evaluated live across 6 random seeds, with
frozen policy weights: an initial batch of 3 seeds at 2 episodes each,
extended with a further 3 seeds at the full 5-episode protocol once a
QoE-calibration issue discovered during this study (Section~IV-B) was
corrected, for 21 fully-logged episodes per arm. The static-at-cap arm
...
across 21 live episodes each, following a fully powered re-evaluation
under the corrected QoE calibration described in
Section~\ref{sec:feasibility}. DQN under the QoE-aware reward sustains
100\% SLA compliance on every slice, in every episode (21/21 episodes
fully compliant). The static baseline, the static-at-cap arm, and DQN
under the SLA-only reward each reach a similar, lower rate instead: 19
of 21 episodes fully compliant for all three, with the 2 non-compliant
episodes in each case concentrated in a single seed rather than spread
evenly across seeds -- consistent with the real seed-to-seed
variability real RF hardware can show even at a fixed seed value.
Fig.~\ref{fig:ceiling} illustrates the underlying mechanism: the
baseline's commanded ceiling stays flat at its starting value for the
whole episode, while DQN raises each slice's ceiling toward its
calibrated maximum within the first few steps and holds it there.

\begin{table}[t]
\centering
\caption{Live SLA compliance (\%), 21 episodes/arm}
\label{tab:results}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
Arm & eMBB & URLLC & mMTC \\
\midrule
Static baseline & 90.6 & 90.6 & 90.6 \\
Static-at-cap & 94.4 & 90.5 & 90.6 \\
DQN (SLA reward) & 92.6 & 90.5 & 90.8 \\
...
the larger, fully powered sample reported here, this edge does not
clearly hold for the SLA-only reward: DQN-SLA and static-at-cap both
reach 19/21 fully-compliant episodes (Fisher exact $p=1.0$), and even
the static baseline reaches the same rate. The QoE-aware reward is the
one variant whose advantage survives the larger sample, with a clean
21/21 record throughout -- though at this sample size its comparison
against the baseline's own, now-improved rate is not itself
statistically significant (Fisher exact $p=0.49$). We report both
rounds rather than only the more favourable one: a fixed allocation's
failure mode is real and shared by every non-learning arm, but whether
the SLA-only reward's learned admission timing meaningfully reduces it
relative to a well-chosen fixed ceiling, as opposed to the QoE-aware
reward's, is closer than our initial, smaller sample suggested.
...
advantage over static control clearly holds up: a clean 21/21
fully-compliant record against 19/21 for every other arm, including
static-at-cap. The SLA-only reward's own edge over a well-chosen fixed
ceiling, which an earlier, smaller-sample round suggested was equally
clear, does not clearly replicate here. The offline congested scenario
...
(21 episodes/arm): DQN under the QoE-aware reward sustained 100\% SLA
compliance throughout, while the SLA-only reward and both static arms
converged to a statistically indistinguishable 19/21 rate. This
revises our own earlier, smaller-sample finding that the SLA-only
reward's collapse-avoidance edge over a best-possible fixed ceiling
was clearly significant ($p=0.0149$) -- at full power that edge holds
for the QoE-aware reward, but not clearly for the SLA-only one
($p=1.0$). The control loop adds negligible latency and model
