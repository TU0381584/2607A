...
Since only one DQN-SLA checkpoint was evaluated above, we additionally
live-evaluated 5 more independently-trained DQN-SLA policies at the
same 21-episode protocol. Despite converging equally well offline,
live records varied substantially (3/5 perfect, one matching the
original 19/21, one collapsing to 13/21, worse than baseline): live
robustness is not predictable from offline convergence alone, and the
checkpoint above is a representative, not best-or-worst-case, draw.
...
static-at-cap, 42/46 for the baseline); the SLA-only reward's own edge,
which a smaller-sample round suggested was equally clear, does not
replicate, and checking 5 more independently-trained DQN-SLA
checkpoints shows why: live robustness varies by training seed in a
way offline convergence does not surface. The offline congested
...
Our future work will (1) identify what distinguishes a robustly-performing
trained checkpoint from a fragile one, since this is not visible in the
offline training reward, (2) recalibrate ceilings and traffic to
