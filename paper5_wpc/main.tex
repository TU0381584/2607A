...
are architecturally untestable on it; we trained a fresh single-agent-DQN checkpoint dimension-matched to the live evaluation path and ran it against a real OAI gNB, 3 UEs, and real per-slice UDP traffic, twice, once from a leftover backlog state and once from a verified-clean baseline.
...
\subsection{Heavier Live Load Against a Static Baseline}
\label{sec:results-m8-load}
The anchor above tests only whether the learned policy's own decisions transfer live, with no comparator. A sharper question is whether its advantage holds against a real baseline once live load increases. We first measured this rig's own live capacity directly rather than assuming it: incrementally attaching UEs while polling swap activity, control-loop responsiveness, and connectivity, 3 and 6 UEs proved stable (zero packet loss, no swap thrashing, 20/20 E2 polls answered), while a 9-UE attempt failed with sustained connectivity loss on a second bring-up. We report 6 UEs, twice the load of the anchor above, as this rig's verified stable ceiling for this comparison rather than the originally intended 9.

At 6 UEs with real per-slice UDP traffic, we ran the same seed-900 single-agent-DQN checkpoint against \texttt{lb\_only}, this project's own fixed-ceiling static baseline, two independent live runs per arm (four episodes total per arm), each checked for a reasonably fresh starting backlog before launch. Fig.~\ref{fig:live-policy-vs-static} reports every episode.

The policy's reward advantage holds decisively: it beats the static baseline in all four episodes (mean reward per step $-6.8$ to $-9.7$) against a static baseline that is worse and identical every time ($-17.5$ in all four episodes, since \texttt{lb\_only} never adapts to traffic). Block precision tells a more honest, mixed story. When the policy blocks at all, it is perfect (1.000, mmTC-only, matching both its offline evaluation and the 3-UE anchor above), decisively better than the static baseline's indiscriminate 0.267--0.395. But in one full run (both episodes), this same checkpoint blocked nothing at all, precision undefined rather than merely low. This is not the already-documented seed-901 checkpoint collapse (a separately-trained checkpoint reported earlier in this project); it is this exact seed-900 checkpoint, previously verified non-collapsed offline and in its first live run, collapsing to always-accept under a different live episode's realized traffic pattern. We report this as new evidence that a learned policy's collapse mode is not always a fixed, checkpoint-level property; it can depend on which live episode is actually realized, a distinction only a live, repeated-run test like this one can surface. Per-slice SLA margins show a real but partial advantage: consistently better for eMBB and mmTC, and mixed for URLLC, where the policy's worse episodes converge on the static baseline's own margin.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig7_live_policy_vs_static.pdf}
\caption{Live 6-UE comparison, learned single-agent-DQN policy (seed-900 checkpoint) vs.\ the static \texttt{lb\_only} baseline, two independent runs per arm (four episodes total per arm), real per-slice UDP traffic throughout. (a)-(c) SLA margin by slice, one linear axis per slice since the three span orders of magnitude. (d) Mean reward per step: the policy wins every episode. (e) Block precision: perfect (1.000) whenever the policy blocks anything, undefined for the two episodes where it blocked nothing (annotated, not plotted as zero), against the static baseline's consistently indiscriminate 0.267--0.395.}
\label{fig:live-policy-vs-static}
\end{figure}

This comparison remains single-gNB throughout, matching every other live result in this paper: it demonstrates that the policy's advantage over a real static baseline survives a heavier live load on the hardware this rig actually supports, not a multi-gNB coordination claim.
...
Two limits bound every claim in this paper. First, every multi-gNB, federated, and disruption result is offline-only, since this rig has one physical gNB, so whether these rankings would survive contact with a live, multi-gNB testbed remains untested, not just under-tested.
