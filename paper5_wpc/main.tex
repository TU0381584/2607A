...
The anchor above uses 3 UEs. To ask whether its finding holds under heavier live load, we ran the same seed-900 single-agent-DQN checkpoint again with 6 UEs attached (two per slice), real per-slice UDP traffic throughout, same episode length. Fig.~\ref{fig:live-3v6} reports both loads directly from their raw logs.

Block precision stays perfect at both loads (1.000, mmTC-only), so the decision logic's correctness does not depend on load in this range. SLA margins do: eMBB is essentially unchanged ($-3\%$), but URLLC and mmTC both worsen substantially at 6 UEs ($+67\%$ and $+65\%$ more negative respectively), while mean reward per step dips only slightly ($-7.2$ to $-7.4$). Doubling live load leaves the policy's targeting intact but costs real margin on the two slices it does not actively protect.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig7_live_3v6.pdf}
\caption{Live single-gNB comparison, same seed-900 checkpoint, 3 UEs vs.\ 6 UEs, real per-slice UDP traffic, same episode length. (a) SLA margin change per slice from 3 to 6 UEs (red: worse; green: better) -- URLLC and mmTC both worsen substantially, eMBB is essentially flat. (b) Mean reward per step at each load.}
\label{fig:live-3v6}
\end{figure}

This comparison remains single-gNB throughout, matching every other live result in this paper: it shows how the policy's own live behaviour changes under heavier load on the hardware this rig actually supports, not a multi-gNB coordination claim.
