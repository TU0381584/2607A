#!/usr/bin/env python3
"""Offline training entrypoint for the LIVE-TRANSFERABLE admission-efficiency
config (saclb_admission_efficiency_live_v1.yaml, Lmax=10/cap=12/4/3, matching
saclb_campaign.yaml exactly). Mirrors train_offline_admission_efficiency.py's
CLI/output layout but uses admission_efficiency_live_env.py's validated
construction (backlog_capacity=30.0, oversub_of_cap=1.2).

Usage:
    python3 experiments/scripts/train_offline_admission_efficiency_live.py \
        --algorithm dqn --reward-mode qoe --episodes 300 --seed 256 \
        --results-dir experiments/results/admission_efficiency_live_offline
"""
import argparse
import sys
...
def main() -> None:
    ...
    ap.add_argument("--results-dir", default="experiments/results/admission_efficiency_live_offline")
    ...
    for s in summaries:
        print(f"[train_offline_admission_efficiency_live] {args.algorithm}/{args.reward_mode} "
              f"seed={args.seed}: {s}")
