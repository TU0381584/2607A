...
def check_loss(netns: str | None) -> float:
    ...
    # Real bug found 2026-09-05: ping prints a DECIMAL percentage whenever
    # the received count isn't a clean fraction of the sent count (e.g. 1/3
    # or 2/3 packets -> "66.6667% packet loss"). The old pattern
    # r"([+-]?\d+)% packet loss" has no \. in its digit class, so \d+ can't
    # cross the decimal point -- re.search then matches the FIRST place a
    # bare digit run sits immediately before "%", which is the post-decimal
    # digits ("6667"), not the real value (66.6667). This produced the
    # previously-observed "anomalous 6667%" readings, which got clamped to
    # 100 (assumed to be a duplicate-reply artifact on an already-broken
    # link) -- but at least once (the postfix_configfix_S0_C1_retest run)
    # this fired on a genuine partial-loss reading (1 of 3 received, high
    # latency) that the clamp then misreported as complete 100% loss,
    # which would have looked identical to the real RLC-exhaustion failure
    # this whole investigation was chasing. Fixed to capture the full
    # decimal value.
